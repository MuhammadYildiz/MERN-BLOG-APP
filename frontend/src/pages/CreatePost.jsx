...
export default function CreatePost() {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            navigate("/posts/post/" + response.data.savedPostBlog._id)
        } catch (error) {
            console.error(error)
        }
    };
    ...
    return (
        ...
        <Helmet><title>Write Post</title></Helmet>
            <Navbar />
            <div className="min-h-[72dvh] flex flex-col justify-start items-center mb-5">
                <h1 className="text-xl font-bold text-gray-950 mb-5" >Post a blog</h1>
                <form onSubmit={(handleCreate)} className="shadow-2xl shadow-gray-900 rounded-2xl bg-gray-800 flex flex-col p-5 text-white w-[350px]  sm:w-[600px] md:w-[750px] lg:w-[80%]">
                    <input type="text" placeholder="Enter blog Title:" value={title}
                        onChange={(e) => setTitle(e.target.value)} onFocus={() => setTitle("")}
                        className="outline-none border border-gray-900  p-1 px-2 m-1 text-black" />
                    <div className="flex justify-between items-center m-1">
                        <input type="file" onChange={(e) => setFile(e.target.files[0])} id="fileInput" className="bg-white text-gray-900 w-80 " />
                    ...
                    <textarea name="" id="" cols="30" rows="10" placeholder="Enter blog Description:" value={desc} onChange={(e) => setDesc(e.target.value)} onFocus={() => setDesc("")}
                        className="outline-none border border-gray-900 m-1 max-h-96 p-2 text-black"></textarea>
                    <button type="submit" className="text-gray-900 bg-white m-1 rounded-md font-bold uppercase hover:bg-blue-100  py-1">Create</button>
                    <button type="button" onClick={toHome} className="text-gray-900 bg-white m-1 rounded-md font-bold uppercase hover:bg-blue-100  py-1">Cancel</button>
                </form>
            </div>
            <Footer />
        </div>
